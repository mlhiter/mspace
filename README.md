...
- Agents shows fixed Codex, Claude Code, and Pi readiness on This Mac, server-derived claimable Worker coverage for the current workspace, and a Connected Workers matrix with liveness and per-engine installation/configuration status. Skills remain a separate tab.
