...
Personal desktop workspaces proactively keep one generic host-local worker ready once auth and workspace selection are available. Electron discovers installed `codex`, `claude`, and `pi` commands without launching them, passes those exact capabilities to the worker, and performs action-level preflight for the mentioned Agent before queueing. The desktop creates and renews a short-lived `msw_...` credential; explicit Codex actions still require the local Codex auth file, while Claude Code and Pi report authentication failures from their adapters. Ordinary startup does not launch Chrome. Browser-backed Tests remain Codex system Workflows and use a separate browser-capable personal Worker advertising `codex`, `browser`, and `chrome_cdp`, so browser preparation cannot interrupt an existing task.
