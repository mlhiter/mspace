...
- The runner persists `agent_profile`, `codex_thread_id`, `codex_turn_id`, `agent_status`, `artifact_dir`, `agent_token`, `cleanup_status`, and `cleaned_at` on `agent_sessions`.
...
- Issue statuses are explicit workflow states: `open`, `in_progress`, `needs_review`, `changes_requested`, `ready_for_test`, `test_in_progress`, `test_passed`, `test_failed`, `blocked`, `failed`, `cancelled`, and `closed`. Legacy `review`, `testing`, `queued`, `running`, `completed`, and `done` normalize into the explicit states. UI surfaces should show readable labels such as `Needs review` and `Test in progress`, not raw underscored values.
...
- Only humans may set a top-level issue to `closed`. Agents may move their scoped issue to review/test/progress/failure states and may close child tasks under their assigned issue; every status transition should be recorded as an actor-authored issue timeline comment and displayed as a one-line status event with from/to badges.
