...
- The issue creation modal, Issue Detail reply composer, and Project runbook editor use `IssueDocumentEditor`, a TipTap-backed Markdown editor in `packages/views/src/issue-document-editor.tsx`, so document writing remains rich while the runner still receives Markdown. Issue/comment editors support direct image upload, paste, and drop through stable `/api/attachments/<id>` Markdown image URLs backed by runner `issue_attachments` rows and rendered as thumbnail node views. Issue creation does not expose a project selector; mspace infers the project from the note.
...
- Review evidence is captured separately in `session_review_evidence`. Keep code diffs and changed files in the Commits tab; the Evidence tab is for commands run, tests, build/deploy results, Kubernetes resources, preview URL, agent summary, risks/follow-ups, and cleanup/retain state tied to the session and source commit.
...
- Project runbooks are agent-discovered project memory stored in SQLite `project_runbooks` with revision history in `project_runbook_revisions`. Projects settings are a full page, not a modal; the runbook is edited through the TipTap Markdown editor, while project rows use a quiet ghost gear icon for settings. Do not reintroduce install/test/build/deploy command form fields. Codex sessions receive the runbook as advisory context and can update it by writing `${MSPACE_SESSION_ARTIFACT_DIR}/project-runbook.md`.
...
- Issue statuses are explicit workflow states: `open`, `in_progress`, `needs_review`, `changes_requested`, `ready_for_test`, `test_in_progress`, `blocked`, `cancelled`, and `closed`. `cancelled` means the issue is closed as not planned; it is not used for a stopped agent session. Legacy `review`, `testing`, `queued`, `running`, `completed`, and `done` normalize into the explicit states, while existing test-data outcome statuses such as `test_passed`, `test_failed`, and `failed` are reset to `open` when the local database is upgraded. UI surfaces should show readable labels such as `Needs review`, `Test in progress`, and `Closed as not planned`, not raw underscored values.
...
- Only humans may set a top-level issue to `closed` or `cancelled`. Agents may move their scoped issue to review/test/progress states, use `blocked` when they cannot proceed, and may close child tasks under their assigned issue; every status transition should be recorded as an actor-authored issue timeline comment and displayed as a one-line status event with from/to badges.
