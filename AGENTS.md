...
- The left workspace menu is the primary place to switch workspaces, create a team workspace for server admins, and switch the desktop UI language between English and Simplified Chinese. Workspace Settings exposes current-workspace automation plus team-only Team access for members/one-time `msi_...` invitations, and workspace runtime registry UI for registration tokens, worker liveness/capabilities, recent runtime tasks, task events, and worker logs.
