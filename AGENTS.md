...
- The control plane owns users, workspaces, workspace membership, mspace auth sessions, GitHub identity, future GitHub App installation state, runtime registration tokens, runtime worker liveness/capability snapshots, and the first server-side runtime task queue. Desktop and runner code should become runtime clients of this service instead of owning collaboration identity.
- GitHub sign-in creates a personal workspace by default. Team collaboration requires an explicit team workspace created from Workspace Settings; personal workspaces keep team Inbox receipts, invitations, runtime worker registration, registered workers, runtime task queues, and Issue Detail Team worker routing disabled.
...
- Inbox is a review-only feed for per-user team issue events. The server control plane stores `issue_events`, `issue_event_receipts`, and `issue_watchers` for team workspaces; the local runner `inbox_items.unread` path remains a personal/local fallback until issue collaboration fully moves behind `server/`.
...
- Issue Detail can explicitly route an agent mention to either the Local runner or a Team worker when the selected workspace is a team workspace. Team sessions write local context, queue an `agent_session` runtime task with repository/session metadata through the control plane, let the Server Worker prepare its own repo cache and worktree, import worker logs into local `session_logs`, capture returned Codex thread/turn ids, and adopt returned source commit metadata, changed files, and diff preview into `issue_change_nodes`. This is an MVP bridge, not a hosted workspace platform; remaining hardening includes stronger artifact transfer, remote credential policy, lower-latency remote cancellation, and Kubernetes provider parity.
- Workspace Settings exposes personal/team workspace mode: personal workspaces show local automation and a create-team-workspace entry; team workspaces expose Team access and Team Runtime registry UI with members, one-time `msi_...` invitations, workspace switching, registration tokens, worker liveness/capabilities, recent runtime tasks, task events, and worker logs beside local automation policy. Keep it as a workspace-level control-plane surface, not an Agents prompt-management view.
