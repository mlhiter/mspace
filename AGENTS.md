...
- The renderer stores the mspace session token under `localStorage["mspace.authToken"]`, a lightweight display identity under `localStorage["mspace.authIdentity"]`, and the selected UI locale under `localStorage["mspace.locale"]`; the display identity is used only to populate local runner `creator*` and `author*` fields until collaboration state moves fully behind the control plane.
...
- Issue Detail routes agent mentions directly through the server control plane for personal and team workspaces. The renderer writes the server comment, then calls `POST /api/workspaces/{workspaceID}/issues/{issueID}/sessions`; the server creates an `agent_session` runtime task with repository/session metadata, workers prepare their own repo cache and worktree, worker logs stay in `runtime_task_logs`, and returned Codex/source metadata is exposed from the runtime task result as session detail and Commits-tab change nodes. The runner must not mirror worker logs/results as source of truth.
- The left workspace menu is the primary place to switch workspaces, create a team workspace, and switch the desktop UI language between English and Simplified Chinese. Workspace Settings exposes current-workspace automation plus team-only Team access for members/one-time `msi_...` invitations, and workspace runtime registry UI for registration tokens, worker liveness/capabilities, recent runtime tasks, task events, and worker logs. Keep it as a workspace-level control-plane surface, not an Agents prompt-management view.
...
- SQLite database path: `~/.mspace/mspace.db`; it is the local runner store for local-only state that has not moved server-side yet: agent profiles, clusters, legacy/local sessions, logs, evidence, handoffs, test environments, runtime artifacts, and legacy attachment rows. Signed-in workspace product data and normal agent session/runtime state live in server Postgres for both personal and team workspaces.
...
- Branch / PR handoff is captured in `issue_handoffs` as an issue-level delivery artifact: one issue should have one current PR, while commits are evidence/source context for that PR. The Commits tab should treat `Source branch` as the PR source identity and de-dupe multiple captured commits on the same branch. The local MVP uses the user's current git, `gh`, and `gitleaks` identity to push the selected issue source branch, run a local secret scan, create the PR or auto-detect an existing PR for that branch, refresh PR state, and include issue metadata, preview URL, commit list, and review evidence summary in the PR body. Source commit capture is always on; Workspace Settings can opt into automatic draft PR creation after source commit capture. The productized direction is a server-owned GitHub App installation token, not a required standalone agent GitHub account.
...
- Session branches may start with an internal `mspace/<issue-short-id>/<session-short-id>` fallback, but source branches shown for PR handoff should prefer the semantic branch name captured from `branch-name.json`.
...
- Project runbooks are agent-discovered project memory stored in server `project_runbooks` with revision history in `project_runbook_revisions`. Projects settings are a full page, not a modal; the runbook is edited through the TipTap Markdown editor, Issue Detail shows a compact sidebar entry that opens a TipTap read-only modal, and project rows use a quiet ghost gear icon for settings. Do not reintroduce install/test/build/deploy command form fields. Codex sessions receive the runbook as advisory context from the server session payload.
- The signed-in Inbox polls server receipt state. The renderer passes the current server token/workspace to the runner through `POST /api/control-plane/session` only for remaining local-runner facilities such as attachments, test environments, PR handoff, and reviewable local issue events; server-owned agent sessions no longer route through a runner bridge. Server-owned issue attachments are still pending.
...
- Vite resolves shared frontend aliases through `apps/desktop/electron.vite.config.ts`: `@mspace/i18n`, `@mspace/ui/components`, `@mspace/ui/lib`, and `@mspace/ui`.
...
- Codex-backed worker sessions should not start or keep development servers running by default, and should not report container-local `localhost` or `127.0.0.1` URLs as user-facing previews. Prefer lint, tests, typecheck, build, or short internal probes unless the user explicitly asks for a mapped preview/live server.
...
- Visible product copy in desktop, UI, and view packages should be locale-aware through `@mspace/i18n` when it belongs to the shell or main workflows. Keep storage values, logs, user-authored content, branch names, commit hashes, Kubernetes identifiers, and runtime protocol fields in their source language unless the user explicitly asks to localize those technical surfaces.
