...
- Runtime registration tokens use the `msw_` prefix and are created from the server control plane. Registered workers report name, mode, version, capabilities, labels, status, load, and heartbeat time through `runtime_workers`. Server-side `runtime_tasks` can be queued by authenticated workspace users, claimed by eligible online workers, cancelled by workspace users, advanced through claimed/running/final states by the claiming worker, and inspected through `runtime_task_events` plus worker-appended `runtime_task_logs`.
- Issue Detail can explicitly route an agent mention to either the Local runner or a Team worker. Team sessions write local context, queue an `agent_session` runtime task with repository/session metadata through the control plane, let the Server Worker prepare its own repo cache and worktree, import worker logs into local `session_logs`, capture returned Codex thread/turn ids, and adopt returned source commit metadata, changed files, and diff preview into `issue_change_nodes`. This is an MVP bridge, not a hosted workspace platform; remaining hardening includes stronger artifact transfer, remote credential policy, lower-latency remote cancellation, and Kubernetes provider parity.
- Workspace Settings exposes the Team access and Team Runtime registry UI: members, one-time `msi_...` invitations, workspace switching, registration tokens, worker liveness/capabilities, recent runtime tasks, task events, and worker logs live beside local automation policy. Keep it as a workspace-level control-plane surface, not an Agents prompt-management view.
