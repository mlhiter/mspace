...
- Runner-owned Issue Detail can explicitly route an agent mention to either the Local runner or a Team worker when the selected workspace is a team workspace. Team sessions write local context, queue an `agent_session` runtime task with repository/session metadata through the control plane, let the Server Worker prepare its own repo cache and worktree, import worker logs into local `session_logs`, capture returned Codex thread/turn ids, and adopt returned source commit metadata, changed files, and diff preview into `issue_change_nodes`. Server-owned issues currently support document collaboration first and guard agent-session actions until the runtime bridge can operate on PG-backed issue ids. This is an MVP bridge, not a hosted workspace platform; remaining hardening includes stronger artifact transfer, remote credential policy, lower-latency remote cancellation, and Kubernetes provider parity.
