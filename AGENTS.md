...
- GitHub sign-in creates a personal workspace by default. Signed-in personal and team workspaces both store product data plus runtime session/task/log/result state in the server control plane. Team workspaces additionally unlock invitations and shared membership.
- Desktop GitHub sign-in starts at `GET /api/auth/github/start`, opens the returned GitHub URL in the browser, lets the server-side callback complete OAuth, then polls `GET /api/auth/github/result?state=...` for a single-use `msp_...` session token. Do not move the GitHub client secret into Electron or workers.
...
- Runtime registration tokens use the `msw_` prefix and are created from the server control plane for personal and team workspaces. Registered workers report name, mode, version, capabilities, labels, status, load, and heartbeat time through `runtime_workers`. Server-side `runtime_tasks` can be queued by authenticated workspace users, claimed by eligible online workers, cancelled by workspace users, advanced through claimed/running/final states by the claiming worker, and inspected through `runtime_task_events` plus worker-appended `runtime_task_logs`.
- Issue Detail routes agent mentions directly through the server control plane for personal and team workspaces. The renderer writes the server comment, then calls `POST /api/workspaces/{workspaceID}/issues/{issueID}/sessions`; the server creates an `agent_session` runtime task with repository/session metadata, workers prepare their own repo cache and worktree, worker logs stay in `runtime_task_logs`, and returned Codex/source metadata is exposed from the runtime task result as session detail and Commits-tab change nodes.
...
- GitHub OAuth should prove identity only. The product session should be an mspace-issued token, and future GitHub repository automation should use GitHub App installation tokens owned by `server/`.
- Keep `server/` independent from Codex runtime dependencies. Issue title suggestion uses deterministic fallback in the server; any LLM-backed issue classification or agent turn must go through worker-claimed runtime tasks.
