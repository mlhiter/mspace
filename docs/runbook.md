...
8. Connect a Codex-capable worker, then verify `test-case-proposals.json` becomes Case suggestions and `test-result.json` updates run items. The Cases list and Case Detail should also show the latest final run item status for each case, and Case Detail / Run Detail should expose structured evidence such as screenshots, assertions, network summaries, DOM snapshots, and raw evidence.
