...
Ordinary personal-workspace startup prepares a Codex-only worker and does not launch Chrome. For browser-backed test batches, including `ui` cases and functional cases that explicitly require platform/session UI entry, the server returns `browser:true` plus `chrome_cdp:true` in the Test Plan or Run detail `requiredCapabilities`; renderer action preflight uses that map instead of reclassifying cases. Electron then prepares a dedicated CDP endpoint and starts a separate `-browser` companion Worker while leaving the Codex-only Worker alive. Both processes reread the same managed credential file, but use different Worker names, capability snapshots, and execution roots; the companion stores its repo cache and workdirs under `<Electron userData>/worker/browser-companion`, avoiding cross-process Git cache/worktree races. Electron first honors `MSPACE_CHROME_CDP_URL` if it is already reachable, otherwise it tries a desktop-managed Chrome profile under `<Electron userData>/worker/browser-profile`, then falls back to an Electron-managed Chromium host under `<Electron userData>/worker/electron-browser-profile` when local Chrome cannot expose CDP. The reachable CDP URL is passed only to the browser-capable Worker as `MSPACE_CHROME_CDP_URL`. If no CDP endpoint can be reached, the browser-required preflight fails and leaves the Codex-only Worker untouched.
...
Issue title suggestion is deterministic server fallback only. Issue creation stores an immediate plain-text draft title and starts final title suggestion without blocking navigation, analysis, or type triage. Current clients mark the draft with `titleSource="plain_text"`; a missing source is the compatibility path for older Markdown-derived drafts. The background update includes `expectedTitle`; if the title changed before the result arrived, the server keeps the current Issue unchanged. A suggestion failure also leaves the draft title intact. When debugging title drift, verify that the create request includes `titleSource`, that the follow-up update includes the exact stored draft as `expectedTitle`, and that no ordinary Issue update rewrites omitted fields. Issue type triage is LLM-backed, but it still runs through the worker queue as `runtime_tasks.kind="issue_type_triage"` with `requiredCapabilities={"codex":true}`. The server validates the worker result before writing the type label. Project-backed issues can also queue one read-only `agent_session` automation marked `issue_analysis` before type triage starts when a matching Codex worker is already online; if the issue was created without a project, attaching a project later tries to queue the same first analysis. That task receives the server-owned `think` skill bundle, runs with a read-only sandbox and source capture disabled, and produces first-pass analysis without requiring a manual `@codex` comment. If no project or worker is available, issue creation still succeeds and the analysis task is skipped.
