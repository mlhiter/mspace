...
For browser-backed test batches, including `ui` cases and functional cases that explicitly require platform/session UI entry, the desktop personal worker also prepares a dedicated CDP endpoint and advertises `browser:true` plus `chrome_cdp:true`. Electron first honors `MSPACE_CHROME_CDP_URL` if it is already reachable, otherwise it tries a desktop-managed Chrome profile under `<Electron userData>/worker/browser-profile`, then falls back to an Electron-managed Chromium host under `<Electron userData>/worker/electron-browser-profile` when local Chrome cannot expose CDP. The reachable CDP URL is passed to the worker as `MSPACE_CHROME_CDP_URL`. If no CDP endpoint can be reached, the worker still starts with Codex-only capability instead of claiming browser execution support.
