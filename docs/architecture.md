...
For Tests setup and execution sessions, the artifact contract is a completion signal as well as evidence. A worker may complete an `agent_session` runtime task from a matching `test-setup-result.json` or `test-result.json` when Codex has produced the artifact but the app-server turn does not emit a final completion notification. A restarted worker may reclaim its own stale `running` task before claiming new queued work, recover the existing session workdir, and submit the artifact-backed final result so the server can reconcile the run without duplicating a freshly running task.
