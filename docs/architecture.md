...
- Workspace issue comments are stored through the server collaboration APIs. Issues may be created before a project is known; Issue Detail can attach an existing project later, and no-project issues remain reviewable but cannot start agent execution, PR handoff, or issue test environments. Server-owned PG-backed issues in team workspaces can start Team worker agent sessions through the runner bridge only after a project is attached: the desktop writes the PG comment, calls `POST /api/server-issues/{issueID}/team-session`, and the runner snapshots issue/project/comment state into SQLite before queueing the control-plane `agent_session` runtime task. Personal workspaces keep Team worker routing disabled.
...
| `POST` | `/api/server-issues/{issueID}/team-session` | Bridge a PG-backed team workspace issue comment into a Team worker `agent_session` runtime task after snapshotting server issue/project/comment state locally. |
...
The control plane implements `workspaces` and `workspace_members`. A GitHub user gets a personal workspace by default. Personal and team workspaces use the server for projects, runbooks, issues, child issue tasks, comments, reactions, labels, and Inbox receipts. Team workspaces additionally unlock shared members, invitations, runtime worker registration, runtime tasks, and PG-backed Issue Detail Team worker routing. The local runner remains the local execution/evidence store for sessions, test environments, handoffs, clusters, attachments, and runtime metadata; team workspace PG-backed issues can queue Team worker sessions through the runner bridge.
...
Issue Detail routes agent mentions by workspace mode. Runner-owned personal workspace execution uses the local runner path. Server-owned PG-backed issues in team workspaces do not offer Local runner execution; they first create the PG comment through the server collaboration API, then call the runner bridge `POST /api/server-issues/{issueID}/team-session` with `runtimeMode: "team"`, workspace id, comment id, issue/project snapshots, child issues, labels, and the selected agent profile. The runner requires its configured control-plane workspace to match, snapshots the server issue/project/comment into SQLite, creates a local `agent_sessions` row with `trigger_comment_id`, queues an `agent_session` runtime task with repository and session metadata, imports worker logs into local session logs, records returned Codex thread/turn ids, and adopts returned source commit metadata, changed files, and diff preview. The Server Worker prepares its own repo cache, session worktree, and artifact directory from the task payload, so a fixed VM, DevBox, or Docker worker can complete the MVP loop without sharing the desktop machine's workdir. The remaining product gaps are stronger artifact transfer, remote credential policy, lower-latency cancellation guarantees, and adding the Kubernetes provider as a second backend without changing the issue/session protocol.
