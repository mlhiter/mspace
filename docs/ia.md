...
- derives a plain-text draft title from the editor document, creates and opens the Issue immediately, and lets the existing worker-backed type triage turn conditionally refine that title later without replacing a human edit or delaying project analysis;
...
- shows issue-level branch / PR handoff state on the Commits tab and sidebar, with personal mode queuing one active Codex PR handoff session with the server-provided `pr-creator` skill to publish the captured source branch, create or find the PR, and record the PR URL from `pull-request.json`; the server refresh path then reads GitHub PR state so `open`, `draft`, `closed`, and `merged` stay current in the UI;
