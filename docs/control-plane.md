...
- repository cache;
- one reusable Issue source worktree for serialized human source Sessions;
- detached per-Session workdirs for system automation and explicit source-Commit tasks;
...
The desktop requires an mspace session before product data is available. In personal mode, Electron ensures one generic host-local Worker and discovers installed Agent executables without launching them. Issue Detail maps `@codex`, `@claude`, or `@pi` to `codex`, `claudeCode`, or `pi`, checks runtime availability with `issueId`, asks Electron to ensure that capability when appropriate, then writes the trigger comment and posts a Session with `agentEngine`. Team workspaces require a connected team Worker. The Server repeats engine, project, runtime-mode, `issueWorkingCopyV1`, active-writer, recovery, liveness, and Worker-storage affinity checks and returns an HTTP `409` conflict when no Worker can claim the source turn. Browser-backed Tests remain detached Codex Workflows and use a separately named browser companion Worker.
...
Runtime provider
  ...
  -> reports name, opaque storage id, mode, version, capabilities, labels, load, and optional Agent diagnostics
...
Server
  ...
  -> lets online workers claim queued tasks that match mode, capabilities, and optional Issue storage affinity
```
...
`runtime_workers.agent_engine_diagnostics` stores only fixed fields: `status`, `reasonCode`, sanitized `version`, and `checkedAt`. Migration 031 adds this JSONB column for Postgres; the SQLite personal store carries the same snapshot field. The server permits only the fixed engine/status/reason enums, valid timestamps, and path-free version text; Pi model reasons are retained only as `pi + unverified + model_available` or `pi + needs_setup + model_unavailable`. Omitted heartbeat diagnostics preserve the locked stored value, while explicit `{}` clears it. `GET /runtime/availability` returns `claimableWorkerCount`, derived by the server with the same workspace mode, status, heartbeat TTL, load, exact capability, optional `issueId` storage-affinity, and working-copy state rules used for claiming. Older Workers remain compatible for detached tasks when they omit diagnostics, but mutable source tasks additionally require `issueWorkingCopyV1:true` and a non-empty opaque `msws_...` storage identity.

The Worker daemon in `worker/` registers, heartbeats, and claims tasks by exact capability. Shared Worker Core prepares repository caches, reusable Issue worktrees, detached automation workdirs, and isolated Session artifacts; it materializes pinned Skills, races required Tests artifacts against engine completion, captures source, and assembles generic results. Human source Sessions reuse `workdirs/<project>/<issue>` on the Server-owned stable branch. System automation and explicit source-Commit tasks continue to use detached `workdirs/<project>/<session>` directories. The Codex adapter uses app-server stdio; Claude Code uses print-mode stream JSON and requires a terminal `result`; Pi uses official RPC, sends `abort` on cancellation, and requires `agent_end`. Missing `agentEngine` maps to Codex only for historical payloads; explicit unknown values fail closed. Docker-backed Workers keep repository caches and worktrees under `/var/lib/mspace-worker`.

Migration 032 adds `issue_working_copies`, Worker storage identity, task storage affinity, and cancellation request state. A source Session transaction creates or locks the Issue working-copy row, rejects a different project or active writer, and queues one `executionMode:"issue_working_copy"` task with the current generation, expected head, stable branch, and `issueWorkingCopyV1` requirement. First claim binds the row to the claiming Worker's storage id. Terminal Worker results include a sanitized working-copy envelope; the Server applies it only when both `activeSessionId` and generation still match. Queued cancellation clears the reservation immediately. Claimed/running cancellation remains a request until the Worker stops the Agent and reports the final clean, dirty, or recovery-required workspace state.
...
PR handoff records live in server `issue_handoffs`. The current implementation records the selected source branch/commit, preview URL, evidence summary, PR URL/state, and handoff state. Personal mode queues a detached Codex `pull_request_handoff` session pinned to the selected source commit; the server provides the `pr-creator` Skill bundle, Codex publishes or reuses the source branch, creates or finds the PR with the authenticated git/GitHub CLI identity available in that personal runtime, writes `${MSPACE_SESSION_ARTIFACT_DIR}/pull-request.json`, and the Server reconciles that artifact into `issue_handoffs`. Only one active PR handoff session may run for an issue at a time, and a completed PR handoff without a valid `pull-request.json` is recorded as a handoff error instead of silently succeeding. The refresh endpoint reads GitHub PR state from the server side and updates `prState`, `prTitle`, `headCommitSha`, and `last_checked_at`, so merged PRs surface as `merged` instead of staying stale. It first uses anonymous REST for public repositories, then falls back to the current user's `gh api` authentication when the local server has `gh` available and the anonymous read cannot see the PR. Workspace GitHub App installation metadata lives in `workspace_github_app_installations` and is exposed as read-only status for Workspace Settings. GitHub App-backed token minting and write-side team PR automation are still future work, but the records themselves remain server-owned.
