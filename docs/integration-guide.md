...
| `POST` | `/api/workspaces/{workspaceID}/issues/{issueID}/handoffs/{handoffID}/refresh` | Refresh the issue handoff record. The server reads GitHub PR state with anonymous REST, falls back to the current user's `gh api` authentication when needed, and normalizes merged PRs to `merged`; personal Codex handoffs still use the session-based PR creation path. |
