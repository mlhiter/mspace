...
| `POST` | `/api/workspaces/{workspaceID}/issues/{issueID}/handoffs/{handoffID}/refresh` | Refresh the issue handoff record. For the personal local path, the server reads GitHub PR state with the current user's `gh api` authentication and normalizes merged PRs to `merged`; personal Codex handoffs still use the session-based PR creation path. |
