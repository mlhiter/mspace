...
| `POST` | `/api/workspaces/{workspaceID}/issues/{issueID}/handoffs/{handoffID}/refresh` | Refresh the server-owned issue handoff record from GitHub through the current user's `gh api` authentication for the personal local path. Personal Codex handoffs still create PRs through the `pull_request_handoff` session path. |
